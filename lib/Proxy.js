...
class Proxy {
  ...
  constructor(port, snapshot, options) {
    this.port = port;
    ...
    return Co(function* () {
      if (!MattermostService.valid(req)) {
        ...
        if (url.pathname === Config.get('auth.oidc.callback.pathname')) {
          ...
          Util.redirect(res, Util.Cookie.for(req).get(self.snapshot.cookie.name));
        }
      }
    });
  }
...
module.exports = new Proxy(Config.get('port'), Config.get('snapshot'), Config.get('proxy'));
