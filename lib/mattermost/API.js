...
const FORBIDDEN_MARSHAL_USER_KEYS = ['_id', 'oid', 'groups', 'image', 'isStaff', 'createdAt', 'updatedAt'];
const DEFAULT_REQUEST_OPTIONS = Object.freeze({
  ...
  finalize(data, message) {
    ...
    if (message.params) {
      message.params = Util.toSnakeCase(message.params);
    }

    return message;
  },
  digest(data) {
    if (data == null || Number.isNaN(data) || typeof data !== 'object') {
      return data;
    }

    if (Array.isArray(data)) {
      return data.map((el) => DEFAULT_REQUEST_OPTIONS.digest(el));
    }
...
function marshalUser(data, message) {
  ...
  const props = { oid: data.oid, groups: data.groups, image: data.image, isStaff: data.isStaff };

  output.props = Object.keys(props).reduce((acc, key) => {
    if (props[key] != null && !Number.isNaN(props[key])) {
...
module.exports = {
  user: {
    ...
    setImage(message, options) {
      const finalize = (data, _message) => digestImage(data.image)
        .then((image) => {
          const formData = new FormData();
          formData.append('image', image, 'image');
          _message.data = formData;
          _message.maxRedirects = 0; // Disable redirects to avoid 'write after end' error
