...
const getOrCreateCredentials = Co.wrap(function* (credentials, first = true) {
  ...
  const root = yield User.find({ username, root: true });
  if (root) {
    return { login_id: root.username, password: root.password };
  }
  ...
  yield new User(Object.assign(user, { password, root: true })).insert();

  return { login_id: user.username, password };
});
...
const onReady = Co.wrap(function* (callback, count = 1) {
  try {
    ...
    const num = count > 10 ? 1 : count;
    // eslint-disable-next-line no-restricted-properties
    setTimeout(() => onReady(callback, num + 1), Math.pow(READY_TIMEOUT, num));
    return;
  }
