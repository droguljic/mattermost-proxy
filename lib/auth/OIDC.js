...
class OIDC {
  ...
  constructor(settings, destination) {
    ...
    this.issuer = new Issuer(Util.toSnakeCase(settings.provider));
    this.client = new this.issuer.Client(Util.toSnakeCase(settings.client));
  }
  ...
  getAuthorizationURL(req, res) {
    ...
    const params = Object.assign(Util.toSnakeCase(settings.authParams), { state });
    const url = Util.parse(req.url);
  ...
  getToken(req, res) {
    ...
    return this.client.authorizationCallback(settings.authParams.redirectUri, url.query, { state });
  }
}
